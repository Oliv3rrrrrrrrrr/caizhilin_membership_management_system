...
export default function EditSoupPage() {
  ...
  useEffect(() => {
    const fetchSoup = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : '获取汤品信息失败');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '更新汤品失败');
    } finally {
      setSaving(false);
    }
  };
  ...
  // 输入变更
  const handleInputChange = (field: keyof UpdateSoupRequest, value: string) => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base lg:text-lg">正在加载汤品信息...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg p-6 sm:p-8 text-center border border-gray-100 dark:border-gray-700">
            ...
            <button
              ...
              className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white px-6 sm:px-8 py-2 sm:py-3 rounded-xl font-medium transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 text-sm sm:text-base"
            >
              返回汤品列表
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">编辑汤品</h1>
                  <p className="text-xs sm:text-sm lg:text-base text-gray-600 dark:text-gray-400 mt-1">修改汤品的详细信息</p>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-3 sm:px-4 py-3 sm:py-4 border-2 rounded-xl focus:ring-4 focus:ring-orange-500/20 focus:border-orange-500 transition-all duration-200 text-sm sm:text-base lg:text-lg ${errors.name
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-3 sm:px-4 py-3 sm:py-4 border-2 rounded-xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 text-sm sm:text-base lg:text-lg ${errors.type
                        ? "border-red-300 dark:border-red-600 bg-red-50 dark:bg-red-900/20"
                        : "border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-500"
                      } text-gray-900 dark:text-white`}
                    placeholder="请输入汤品类型"

...
import { getMembershipById, updateMembership } from '@/services/membershipService';
import { MembershipResponse, UpdateMembershipRequest } from '@/types/membership';
...
export default function EditMembershipPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.remainingSoups === undefined || formData.remainingSoups < 0) {
      newErrors.remainingSoups = '剩余汤品数量不能为负数';
    }

    if (formData.remainingSoups !== undefined && formData.remainingSoups > 1000) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateMembership(membershipId, formData, token);
      router.push(`/memberships/${membershipId}?success=updated`);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : '更新会员信息失败');
    } finally {
      setSaving(false);
    }
  };
  ...
  // 处理输入变化
  const handleInputChange = (field: keyof UpdateMembershipRequest, value: string | number) => {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">编辑会员</h1>
                  <p className="text-xs sm:text-sm lg:text-base text-gray-600 dark:text-gray-400 mt-1">
                    编辑会员 {membership.name} 的信息
                  </p>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pt-6 sm:pt-8 border-t border-gray-200 dark:border-gray-700">
              ...
              <button
                type="submit"
                disabled={saving}
                className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 disabled:from-gray-400 disabled:to-gray-500 text-white rounded-xl transition-all duration-200 flex items-center justify-center font-medium shadow-lg hover:shadow-xl transform hover:scale-105 disabled:transform-none disabled:cursor-not-allowed cursor-pointer text-sm sm:text-base"
              >
                {saving ? (
                  <>
                  ...
                  <>
                    <FiSave className="mr-2 text-sm sm:text-base" />
                    保存修改
                  </>
                )}
              </button>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <ul className="text-blue-700 dark:text-blue-300 space-y-1 text-xs sm:text-sm">
                <li>• 会员姓名和手机号将用于系统识别和联系</li>
                <li>• 卡号不可修改，由系统自动生成</li>
                <li>• 发卡日期不可修改，记录会员注册时间</li>
                <li>• 卡类型决定剩余汤品数量，350卡为10次，500卡为20次，980卡为40次</li>
                <li>• 修改手机号时系统会验证唯一性</li>
              </ul>
            </div>
          </div>
        </div>
